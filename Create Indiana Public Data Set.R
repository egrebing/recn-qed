...
enrl <- bind_rows(enrl.15, enrl.16, enrl.17, enrl.18, enrl.19, enrl.20, enrl.21) %>%
  ...
  select(ST_SCHID, Corp_Name, Schl_Name, Year, TOTAL_ENROLLMENT, Grade_6:Grade_12, Grade_13_or_Adult = `Grade_12+/Adult`) %>%
  filter(ST_SCHID %in% survey.matching.data$ST_SCHID) %>%
  pivot_longer(cols = Grade_6:Grade_13_or_Adult) %>%
...
recn.qed.matching.data <- enrl %>%
  left_join(survey.matching.data, by = "ST_SCHID") %>%
  ...
  arrange(desc(TREAT), Cohort, NAME, Year) %>%
  mutate(Time = case_when(
    Cohort == "Tier 1" ~ Year - 2019,
    Cohort == "Tier 2" ~ Year - 2020,
    Cohort == "Tier 3" ~ Year - 2021,
    TRUE ~ -999),
    PYr = as.integer(Cohort == "Tier 1" & Time == 1),
    TYr1 = as.integer((Cohort == "Tier 1" & Time == 2) | (Cohort %in% c("Tier 2", "Tier 3") & Time == 1)),
    TYr2 = as.integer((Cohort == "Tier 1" & Time == 3) | (Cohort %in% c("Tier 2", "Tier 3") & Time == 2)),
    TYr3 = as.integer((Cohort == "Tier 1" & Time == 4) | (Cohort %in% c("Tier 2", "Tier 3") & Time == 3))) %>%
  select(ST_SCHID, Year, NAME_CCD = NAME, TREAT, Cohort, Time, contains("Yr"), everything())

#Write to file
fwrite(recn.qed.matching.data, "RECN QED Matching Data Set v1.5.22.csv", row.names = F, na = "")
